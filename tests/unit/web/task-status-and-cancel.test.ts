...
describe("task status details and cancel actions", () => {
  ...
  it("shows production lanes, stuck-task signals, ETA, heartbeat, and capacity guidance", async () => {
    ...
    await waitFor(() => {
      ...
      expect(text).toContain("业务需处理")
      expect(text).toContain("业务队列")
      expect(text).toContain("需处理：卡住 1 / 失败 1")
      expect(text).toContain("超过 10 分钟无进展")
      expect(text).toContain("先查看素材和失败原因，再恢复卡住任务")
      expect(text).toContain("等待人工处理")
      expect(text).toContain("已有成片")
      expect(text).toContain("不等同于已完成数量")
      expect(text).not.toContain("约 731 小时")
      expect(text).toContain("Provider failed task失败")
      expect(text).toContain("问题类型 接入方超时")
    })
  })
